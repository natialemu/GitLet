...
public class InitializedState implements GitLetStateMachine{
    ...
    public InitializedState(GitVCS gitLet, CommitTree tree){
        this.gitLet = gitLet;
        this.tree = tree;
    ...
    public Void rm(String filename) {
        ...
        if(workingDirectory.contains(filename)){
            ...
            if (lastCommit.exists(filename)) {
                List<FileInfo> fileInfos = lastCommit.getFiles();
                for (int i = 0; i < fileInfos.size(); i++) {

                    FileInfo cFilename = fileInfos.get(i);
                    if (cFilename.getFilename().equals(filename)) {
                        cFilename.tagForRemoval();
    ...
    public Void checkout(String filename) {
        ...
        RandomFileGenerator.copyFile(fileFromLastCommit,inputFile);
    ...
    public Void reset(int commitId) {
        ...
        for(FileInfo fi: commit.getFiles()){
            ...
            if(output.exists()){
                ...
                RandomFileGenerator.copyFile(serializedFileOutput,output);
            }
        ...
        for(FileInfo fileInfo: lastCommitOfGivenBranch.getFiles()){
            if(tree.fileHasBeenModified(fileInfo.getFilename(),brachName,splitPoint) && tree.fileHasBeenModified(fileInfo.getFilename(),tree.getCurrentBranch(),splitPoint)){
                ...
                */
                ...
                RandomFileGenerator.copyFile(source,desitnation);
            }
        }
        gitLet.getSerializer().serializeCommitTree(tree);
        return null;
    ...
    private void createNewFile(String filename, String serializedFileName) {
        try {
            ...
            RandomFileGenerator.copyFile(source,newFile);
